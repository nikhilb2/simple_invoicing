...
import type { CompanyProfile, DashboardSummary } from '../types/api';
    ...
    <div className="page-grid">
      ...
      <section className="stats-grid">
        <article className="stat-card">
          ...
          <p className="stat-card__value">{loading ? <span className="skeleton" style={{ width: '60px', height: '38px', display: 'inline-block' }} /> : summary?.product_count ?? 0}</p>
          <p className="muted-text">Products available for quoting and invoicing.</p>
        </article>
        <article className="stat-card">
          <p className="eyebrow">Stock units</p>
          <p className="stat-card__value">{loading ? <span className="skeleton" style={{ width: '80px', height: '38px', display: 'inline-block' }} /> : summary?.total_inventory_units ?? 0}</p>
          <p className="muted-text">Total quantity currently registered across inventory rows.</p>
        </article>
        <article className="stat-card">
          <p className="eyebrow">Low stock</p>
          <p className="stat-card__value">{loading ? <span className="skeleton" style={{ width: '40px', height: '38px', display: 'inline-block' }} /> : summary?.low_stock_count ?? 0}</p>
          <p className="muted-text">Rows at {(summary?.low_stock_threshold ?? 5)} units or less that likely need replenishment.</p>
        </article>
        <article className="stat-card">
          <p className="eyebrow">Invoice value</p>
          <p className="stat-card__value">{loading ? <span className="skeleton" style={{ width: '120px', height: '38px', display: 'inline-block' }} /> : formatCurrency(summary?.active_invoice_total ?? 0, activeCurrencyCode)}</p>
          <p className="muted-text">Combined gross amount across all active invoices.</p>
        </article>
      </section>
      ...
      <section className="content-grid">
        <article className="panel stack">
          <div className="panel__header">
            ...
            <div className="status-chip">{summary?.tracked_inventory_rows ?? 0} tracked rows</div>
          </div>
          ...
          <div className="table-list">
            ...
            {!loading && (summary?.tracked_inventory_rows ?? 0) === 0 ? (
              <EmptyState 
            ...
            {!loading
              ? (summary?.low_stock_items ?? []).map((row) => (
                    <div key={row.product_id} className="table-row">
                      <div className="table-row__meta">
                        <strong>{row.product_name}</strong>
                        <span className="table-subtext">Product ID #{row.product_id}</span>
                      </div>
                      <span className={`pill ${row.quantity <= (summary?.low_stock_threshold ?? 5) ? 'pill--low' : 'pill--ok'}`}>{row.quantity} units</span>
                      <span className="table-subtext text-right">{row.quantity <= (summary?.low_stock_threshold ?? 5) ? 'Needs attention' : 'Stable'}</span>
                    </div>
                  ))
              : null}
          </div>
        ...
        <article className="panel stack">
          ...
          <div className="invoice-list">
            ...
            {!loading && (summary?.recent_invoices ?? []).length === 0 ? (
              <EmptyState 
              ...
              ? (summary?.recent_invoices ?? []).map((invoice) => (
                  <div key={invoice.id} className="invoice-row">
                    <div className="invoice-row__meta">
                      <strong>{invoice.ledger_name || 'Unknown ledger'}</strong>
