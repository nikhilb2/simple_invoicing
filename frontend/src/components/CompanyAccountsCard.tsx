      ...
      <section className="content-grid" style={{ alignItems: 'start' }}>
        <article className="panel stack">
          <div className="panel__header">
            <div>
              ...
              <h2 className="nav-panel__title">Add account</h2>
            </div>
          </div>
          ...
          {isAdmin ? (
              ...
              <div className="field-grid">
                <div className="field">
                  ...
                  <select
                    ...
                    onChange={(event) => {
                      const nextType = event.target.value as CompanyAccountType;
                ...
                {createDraft.account_type === 'bank' ? (
                  <>
                    ...
                    <div className="field">
                      <label htmlFor="new-account-branch">Branch name</label>
                      <input
                        id="new-account-branch"
                        className="input"
                        value={createDraft.branch_name}
                        onChange={(event) => setCreateDraft((current) => ({ ...current, branch_name: event.target.value }))}
                    ...
                    <div className="field" style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingTop: '22px' }}>
                      <input
                        ...
                        type="checkbox"
                        checked={createDraft.display_on_invoice}
                        onChange={(event) => setCreateDraft((current) => ({ ...current, display_on_invoice: event.target.checked }))}
                      />
                      <label htmlFor="new-account-display-on-invoice" style={{ marginBottom: 0, cursor: 'pointer' }}>
            ...
            <div className="empty-state">Only admin and manager roles can add or edit accounts.</div>
          )}
        </article>
        ...
        <article className="panel stack">
          <div className="panel__header">
            <div>
              <p className="eyebrow">Cash and bank</p>
              <h2 className="nav-panel__title">Available accounts</h2>
            </div>
            <div className="status-chip">{accounts.length} total</div>
          </div>
                  ...
                  <div className="field-grid">
                      ...
                      <select
                        ...
                        onChange={(event) => {
                          ...
                          patchDraft(account.id, {
                            account_type: nextType,
                            bank_name: nextType === 'cash' ? '' : draft.bank_name,
                            branch_name: nextType === 'cash' ? '' : draft.branch_name,
                    ...
                    {draft.account_type === 'bank' ? (
                      <>
                        ...
                        <div className="field">
                          <label htmlFor={`account-branch-${account.id}`}>Branch name</label>
                          <input
                            id={`account-branch-${account.id}`}
                            className="input"
                            value={draft.branch_name}
                            onChange={(event) => patchDraft(account.id, { branch_name: event.target.value })}
                            disabled={!isAdmin}
                          />
                        </div>
                        <div className="field">
                        ...
                        <div className="field" style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingTop: '22px' }}>
                          <input
                            id={`account-display-on-invoice-${account.id}`}
                            type="checkbox"
                            checked={draft.display_on_invoice}
                            onChange={(event) => patchDraft(account.id, { display_on_invoice: event.target.checked })}
                            disabled={!isAdmin}
                          />
                          <label htmlFor={`account-display-on-invoice-${account.id}`} style={{ marginBottom: 0, cursor: isAdmin ? 'pointer' : 'default' }}>
                            Display on invoice PDF
                          </label>
                        </div>
                      </>
